...
class DittofeedIframe extends Component {
    ...
    setup() {
        ...
        this.dittofeedUrl = "http://localhost:3210";
        this.path = this.props.context && this.props.context.dittofeed_path;
    ...
    get computedUrl() {
        if (this.dittofeedUrl && this.path) {
            return this.dittofeedUrl + this.path;
        }
        return this.dittofeedUrl;
    }
    ...
    async loadDittofeedConfig() {
        try {
            ...
            if (config.dittofeed_url) {
                this.dittofeedUrl = config.dittofeed_url;
            }
        } catch (error) {
    ...
    handleMessage(event) {
        console.log("Message from Dittofeed:", event.data);
    }
}
